...
public enum RuleSetEnglish implements IRule, ILanguageRuleSet{
	 ...
	 */
		...
		SPACE("_","Spaces", " ",EnglishRulePrecedences.SPACE, "Encode Spaces"),
		/* TWO letter [re]bate */
		RE("R1","Re","\\bre",EnglishRulePrecedences.PRFIX_SUFFIX_TWO_CHARS, "[Re]course but not windb[re]aker" ),
		...
		/* three letter (pre) */
		PRE("P1","Pre","pre(?=["+RuleSetEnglish.CONSONANTS+"])",EnglishRulePrecedences.PRFIX_SUFFIX_THREE_CHARS, "[pre]tentious" ),
		ING("I1","ing","ing",EnglishRulePrecedences.PRFIX_SUFFIX_THREE_CHARS, "walk[ing]" ),
		...
		KN("M0","KN","kn",EnglishRulePrecedences.DOUBLE_CHAR, "KN->N->M0" ),  //capture 'kn' but encode only 'n'
		
		WR("-","WR","w(?=r)",EnglishRulePrecedences.IGNORE_TWO_CHAR, "wr* -> ignore" ),  //capture [wr]ecked, []
		WHO("-","Who","who",EnglishRulePrecedences.THREE_CHARS, "*who* -> ignore" ),  //capture [wr]ecked, []
		W("W0","W","w",EnglishRulePrecedences.SINGLE_CHAR, "W->W0" ),  //[w]alk [w]ork[w]eek
		WI("-","answer","(?<=ans)w(?=er)",EnglishRulePrecedences.FOUR_CHARS, "answer" ),  //make ans[w]er silent w
		WI2("-","sword","(?<=s)w(?=ord)",EnglishRulePrecedences.FOUR_CHARS, "answer" ),  //make ans[w]er silent w
	//	
		...
		BP("B0","BP,","(b|p)",EnglishRulePrecedences.SINGLE_CHAR, "BFPV->B0"),
		FV("F0","FV,","(f|v)",EnglishRulePrecedences.SINGLE_CHAR, "BFPV->B0"),
...
//	
	...
	//System.err.println();
		...
		RuleSetEnglish(String soundExCode,String name, String patternString, EnglishRulePrecedences precedenceIndex, String description ){
		...
		public  ArrayList<IRule> toArrayList(){
			...
			for(RuleSetEnglish r: RuleSetEnglish.values()) {
				arrayList.add(r);
			}
